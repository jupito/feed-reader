...
def p(*values, **kwargs):
    """Printer."""
    #values = [v.encode('utf_8') for v in values]
    print(*values, **kwargs)
...
def print_bottom(ids=None):
    p('<div id="bottom">')
    if ids:
        p(html.href(link_markread(ids), 'Mark these read'))
    p('</div>')

def show_categories(db):
    p(html.head('Categories', SHEET))
    print_top()
...
def show_feeds(db):
    ...
    p(html.head('Feeds (%s)' % (args['cat'] or 'all'), SHEET))
    print_top()
    p('<div id="feeds">')
    for f in feeds:
...
def print_entryinfo(e, f):
    ...
    p('<div class="entryinfo">')
    p(' &mdash; '.join([updated, cat, feed + flink]))
    p('</div>')

def print_title(x):
    p('<div class="title">')
    p(html.href(x['link'], x['title']))
    p('</div>')
...
def print_enclosure(e):
    if not e['enc_url']:
        return
    p('<div class="enclosure">')
    p(html.href(e['enc_url'],
...
def print_entry(e, f, alt=False):
    p('<div class="%s">' % ('entry_alt' if alt else 'entry'))
    print_entryinfo(e, f)
    print_title(e)
    print_description(e)
    print_enclosure(e)
...
def show_entries(db):
    ...
    p(html.head('%i in %s entries' % (n, 'all' if any else 'unread'), SHEET))
    print_top(ids)
    p('<div id="entries">')
...
def redirect(db):
    ...
    if entries:
        e = entries[0]
        p(html.head('Redirecting...', SHEET, e['link']))
        p('Redirecting to:')
        f = db.get_feed(e['feed_id'])
        print_entry(e, f)
        db.set_progress(e['id'], 1)
    else:
        p(html.head('Cannot redirect', SHEET))
        p('No unread entries.')
    p(html.tail())
